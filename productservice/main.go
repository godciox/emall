...
import (
	...
	_const "productservice/const"
	db "productservice/db/sqlc"
	"productservice/handler"
	"productservice/httphandler"
...
func main() {
	...
	src := web.Router("/", &httphandler.UploadController{})
	src.Router("/get", &httphandler.DownloadProductImageController{})
	// Create service
	srv := micro.NewService(
		micro.Server(grpcs.NewServer()),
		micro.Client(grpcc.NewClient()),
		micro.Registry(config.NewRegistry()),
		//micro.Server(microweb.Server()),
	)
	opts := []micro.Option{
	...
	// Register handler
	if err := pb.RegisterProductCatalogServiceHandler(srv.Server(), new(handler.ProductService)); err != nil {
		logger.Fatal(err)
	}
	...
	db.DB = conn
	web.BeeApp.Cfg.WebConfig.AutoRender = false
	//// Run service
	go src.Run(":8090")
